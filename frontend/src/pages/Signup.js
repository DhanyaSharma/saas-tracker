...
export default function Signup({ onAuth, goToLogin, dark, setDark }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res  = await axios.post(`${base}/signup`, { name: form.name, email: form.email, password: form.password });
  ...
  return (
    <div style={s.page}>
      ...
      <div style={s.card}>
        ...
        <p style={s.sub}>Start tracking subscriptions for free</p>
        ...
        <form onSubmit={handleSubmit} style={s.form}>
          <div style={s.field}>
            ...
            <input name="name" value={form.name} placeholder="John Doe" onChange={handleChange} required style={s.input}/>
          </div>
          <div style={s.field}>
          ...
          <div style={s.field}>
            ...
            <input name="password" type="password" value={form.password} placeholder="Min. 6 characters" onChange={handleChange} required style={s.input}/>
          </div>
          <div style={s.field}>
            <label style={s.lbl}>Confirm Password</label>
            <input name="confirm" type="password" value={form.confirm} placeholder="••••••••" onChange={handleChange} required style={s.input}/>
          </div>
        ...
        <p style={s.switch}>
          ...
          <span style={s.link} onClick={goToLogin}>Sign in</span>
        </p>
      </div>
    </div>
  );
...
const s = {
  ...
  sub:      { fontSize: 14, color: "var(--text3)", marginBottom: 24 },
  err:      { background: "var(--red-bg)", color: "var(--red)", border: "1px solid var(--red-bd)", borderRadius: 9, padding: "10px 14px", fontSize: 13, marginBottom: 14, animation: "fadeIn .2s ease" },
  ...
  switch:   { textAlign: "center", fontSize: 13, color: "var(--text3)", marginTop: 22 },
